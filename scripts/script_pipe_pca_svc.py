...
for train_index, val_index in cv.split(X, y):
    ...
    scoring = {'f1': 'f1', 'MCC': make_scorer(metrics.matthews_corrcoef)}
    estimator = GridSearchCV(pipe, params_grid, cv=2, n_jobs=-1, scoring=scoring, refit='MCC', return_train_score=True)
...
#todo: unknown prediction not working yet
print(un_pred_lst)
print("unknown:\t\t%.2f (+-%.2f)" % (np.mean(un_pred_lst), np.std(un_pred_lst)))

MyFile=open('un_pred_lst.txt','w')
MyFile.writelines(un_pred_lst)
MyFile.close()